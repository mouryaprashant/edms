  ...
  return <section className="ui-card overflow-hidden">
    ...
    <div className="overflow-x-auto custom-scrollbar">
      <table className="min-w-full text-left text-sm">
        <tbody className="divide-y divide-slate-100 bg-white">{filteredStations.length === 0 ? <tr><td colSpan={canEdit ? 3 : 2} className="px-6 py-8 text-center text-xs text-slate-400">No stations match the current search.</td></tr> : filteredStations.map((st) => { const actualIdx = section.stations.findIndex((x) => x.id === st.id); return <StationRow key={st.id} station={st} index={actualIdx} isFirst={actualIdx === 0} isLast={actualIdx === section.stations.length - 1} canEdit={canEdit} canDelete={canDelete} isOpen={openStationIds.has(st.id)} onToggle={onToggleStation} onViewHistory={onViewHistory} onEdit={(stId) => onEditStation(section.id, stId)} onDelete={(stId) => onDeleteStation(section.id, stId)} onMove={(stId, dir) => onMoveStation(section.id, stId, dir)} />; })}</tbody>
      </table>
    </div>
  </section>;
}
